...
class LokiLogger:
    ...
    def check_health(self):
        ...
        if not loki_url:
            self.console_logger.warning(f"⚠️  {Fore.RED}LOKI_URL n'est pas défini. Les logs ne seront pas envoyés à Loki.{Fore.RESET}")
            return False
        ...
        try:
            ...
            if response.status_code < 500:  # Loki répond, même si c'est une erreur de requete
                self.console_logger.info(f"✅ {Fore.GREEN}Loki est accessible et prêt à recevoir les logs.{Fore.RESET}")
                return True
            else:
    ...
    def __init__(self, name="app_logger"):
        ...
        self.logger.setLevel(logging.INFO)
        self.redact = True
        self.sensitive_fields = os.getenv("LOKI_SENSITIVE_FIELDS", "").split(",") if self.redact else []
        ...
        if self.check_health():
            ...
            self.console_logger.warning(f"⚠️  {Fore.YELLOW}Le handler Loki n'a pas été ajouté en raison de problèmes de santé. Les logs ne seront pas envoyés à Loki.{Fore.RESET}")
